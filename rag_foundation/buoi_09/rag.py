"""rag.py — SNAPSHOT baseline cho Buổi 09 (sao chép nguyên trạng từ Buổi 08).

NGUỒN SNAPSHOT: `rag_foundation/buoi_08/rag.py`
SHA-256 bản gốc và bản sao: f41761149ead0f3630307f0ec73b914e40168af9db2a22b8035e567534a251b5
(hai hash trùng nhau — chứng minh copy nguyên trạng, chưa sửa logic)

Buổi 09 KHÔNG import runtime từ thư mục Buổi 08. Mọi thứ Buổi 09 cần đều lấy
qua bản snapshot này. Vì `BASE_DIR` tính theo `Path(__file__).resolve().parent`,
bản sao tự dùng `.env` và `storage/` riêng của `buoi_09/`.

--- Docstring gốc từ Buổi 08 giữ nguyên bên dưới ---

rag.py — Semantic RAG baseline cho Buổi 08 (sao chép nguyên trạng từ Buổi 07).

NGUỒN: đây là bản sao của `rag_foundation/buoi_07/rag.py` (đã hoàn thiện và
nghiệm thu ở Buổi 07 — loader/validator, Gemini embedding, ChromaDB persistent
index, semantic retrieval + confidence gate + generation + citation, 69 test
case PASS). Buổi 08 dùng lại nguyên module này làm "semantic baseline" — tầng
lexical (BM25), hợp nhất RRF, cross-encoder reranker và answer pipeline nâng
cao được viết thêm trong `advanced_rag.py`, KHÔNG sửa file này.

Vì dùng `Path(__file__).resolve().parent` để tính `BASE_DIR`, bản sao này tự
động dùng `.env` và `storage/` riêng của `rag_foundation/buoi_08/` — không
đụng `.env` hay `storage/chroma/` thật của Buổi 07.

Pipeline (không đổi so với Buổi 07):

    Chunks JSON của Buổi 05 (chỉ đọc)
        -> validate
        -> Gemini embedding
        -> ChromaDB persistent index (của Buổi 08)
        -> semantic retrieval top-k
        -> confidence gate theo RAG_MAX_DISTANCE
        -> Gemini tổng hợp câu trả lời có grounding
        -> citation map từ metadata thật

Xem SPEC_buoi_08.md mục "Quan hệ với Buổi 05 và Buổi 07" và mục "Semantic
candidate contract" để biết Buổi 08 dùng lại đúng những hàm nào của module
này (`load_chunks`, `load_config`, `embed_documents`, `embed_query`,
`collection_name`, `_chroma_client`, `retrieve`, ...).

CLI của riêng module này (kế thừa nguyên trạng, dùng để chẩn đoán baseline):
...
CLI chính của Buổi 08 (BM25/hybrid/rerank/compare/query nâng cao) nằm ở
`advanced_rag.py`, không nằm ở đây.
"""
