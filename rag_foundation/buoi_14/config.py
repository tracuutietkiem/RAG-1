...
# ---------------------------------------------------------------- RBAC (Buoi 15)
# Danh sach vai tro hop le doc tu roles.json (single source of truth), tranh
# hard-code / go sai ten role rai rac trong nhieu file.
import json  # noqa: E402

ROLES_JSON = BASE_DIR / "roles.json"
CHUNKS_SECURE_CSV = PROCESSED_DIR / "chunks_secure.csv"
SECURITY_AUDIT_REPORT = OUTPUTS_DIR / "security_audit_report.md"
LAB_SESSION_SECURE = "buoi_15"


def _load_roles_config() -> dict:
    if not ROLES_JSON.exists():
        return {
            "role_names": ["Admin", "HR", "Risk_Manager", "Staff", "Guest"],
            "default_role": "Guest",
...
_ROLES_CFG = _load_roles_config()
ALL_ROLES: list[str] = list(_ROLES_CFG.get("role_names", []))
DEFAULT_ROLE: str = _ROLES_CFG.get("default_role", ALL_ROLES[-1] if ALL_ROLES else "Guest")
CLASSIFICATION_RULES: dict = _ROLES_CFG.get("classification_rules", {})


def validate_roles(roles) -> list[str]:
    """Chuan hoa + kiem tra danh sach role dau vao, chan typo som thay vi
    de loi am tham lam sai ket qua loc quyen."""
    ...
    for r in roles or []:
        ...
        if r not in ALL_ROLES:
            raise ValueError(
                f"Vai tro khong hop le: {r!r}. Cac vai tro hop le: {ALL_ROLES}"
            )
        if r not in out:
            out.append(r)
    return out
