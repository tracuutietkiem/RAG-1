"""
BUOI 19 - PROMPT 2: UC3 - AI Compliance Checker, PHIEN BAN DUAL-PROVIDER
(Ollama local / Gemini cloud).

Ban CAP NHAT cua buoi_18/scripts/compliance_checker.py - GIU NGUYEN 100% rule
engine (regex nguong so, BM25 cua buoi_14), RBAC fail-closed, cau truc output,
CHI THAY doi noi goi LLM tuy chon (llm_refine_conflict) sang dung
`llm_provider.call_llm()` (Ollama/Gemini). buoi_18/scripts/compliance_checker.py
GIU NGUYEN, KHONG bi sua - day la ban song song cho kien truc Docker/Local AI
Buoi 19. DOMAIN_MAP duoc IMPORT THANG tu buoi_18/scripts/data_catalog_b18.py
(khong copy lai).
...
from data_catalog_b18 import DOMAIN_MAP  # noqa: E402  (TAI SU DUNG nguyen ban tu buoi_18, khong copy)

# Doi LOG_PATH ve outputs/ CUA BUOI 19 (khong lam lan voi log buoi_17/18).
...
def rbac_filter(df: pd.DataFrame, user_role: str | None) -> pd.DataFrame:
    """Loc TRUOC retrieval theo allowed_roles - fail-closed."""
    if not user_role:
        return df
...
def llm_refine_conflict(citation_a: str, text_a: str, citation_b: str, text_b: str) -> dict | None:
    """Tuy chon: goi LLM (Ollama/Gemini) CHI voi 2 doan van ban that. Tra ve
    None neu khong co ket qua/loi -> giu CHUA_DU_BANG_CHUNG."""
    prompt = (
    ...
    try:
        ...
        if data.get("has_conflict"):
            ...
            return {
                ...
                "method": f"llm_assisted_{provider}", "confidence": 0.7,
            }
        return {
            "classification": "KHONG_XUNG_DOT", "conflict_type": None, "severity": None,
        ...
        print(f"[CANH BAO] LLM refine that bai/du lieu khong hop le ({type(exc).__name__}: {exc}). "
              "Giu CHUA_DU_BANG_CHUNG.", file=sys.stderr)
        return None
...
def run_compliance_check(
    ...
    for _, irow in internal_all.iterrows():
        ...
        if result.get("method", "").startswith("llm_assisted"):
            llm_calls_used += 1
...
def main() -> None:
    ...
    lines = [f"# Buổi 19 — Compliance Conflict Report (PROMPT 2 / UC3, provider={provider})\n"]
